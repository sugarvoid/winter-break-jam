...
func _spawn_ice_sickle(pos: Vector2 = $Position2D.global_position, rot: int = 0) -> void:
...
func spawn_hazard(sec: int) -> void:
	match sec:
		...
		4:
			_spawn_ice_sickle()
		6:
			_spawn_ice_sickle($Position2D2.global_position)
			_spawn_ice_sickle($Position2D.global_position)
		12:
			_spawn_wave(p_SWTopLeft)
		15:
